...
SELECT title , CONCAT(last_name, ' ',first_name ) AS senior FROM employee
ORDER by title DESC
LIMIT 1;
...
SELECT billing_country AS most_country_invoice, COUNT(billing_country) AS total_count FROM invoice
GROUP BY billing_country 
ORDER BY total_count DESC;

-- 3. What are top 3 values of total invoice?

SELECT total AS top_3_values FROM invoice
...
SELECT c.customer_id , CONCAT(c.first_name,' ',c.last_name) AS customer_name , SUM(i.total) AS total
FROM customer AS c
JOIN invoice AS i
...
-- we can solve this problem with the help of IN query and DISTINCT function
-- we can solve solve this problem by using two method;
...
-- There are many ways to solve this problem but i will solve it 2 methods
...
-- in this problem we will apply COMMON TABLE EXPRESSION which is temporary table
...
-- first method by using CTE

WITH popular_genre AS 
(
	SELECT COUNT(invoice_line.quantity) AS purchases , customer.country , genre.name, genre.genre_id,
	ROW_NUMBER() OVER(PARTITION BY customer.country ORDER BY COUNT(invoice_line.quantity) DESC) AS rawno
	FROM invoice_line
	JOIN invoice 
	ON invoice.invoice_id = invoice_line.invoice_id
	JOIN customer 
...
SELECT * FROM popular_genre WHERE rawno <= 1


-- 2nd method by using RECURSIVE 
